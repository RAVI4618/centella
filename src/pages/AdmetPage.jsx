...
const AdmetPage = () => {
    ...
    const fetchprojectsList = (uname) => {
        ...
        fetch(`${baseurl}/projectManagement/api/projects/get-user-projects?userName=${uname}&productId=2`, {
            headers: defaultheaders
        })
        ...
        () => [
            {
                ...
                onCellClicked: ({ data }) => {
                    navigate(`/admet/projectdetail/${data.id}`, { state: { projectName: data.projectName, projectDescription: data.projectDescription, id: data.id } });
                },
...
<p className="mt-2 flex flex-row">
  ...
  <img src="https://cdn-icons-png.flaticon.com/128/8591/8591654.png" height={20} width={20} alt="" className="mt-1" />
    ...
    <button
        ...
        className="bg-[#8D6CBB] hover:bg-[#735AA7]   font-medium text-white mb-2 rounded-[7px] h-[40px] w-[126px] flex items-center justify-center"
    >
        <span className="text-3xl bold"><img src='./plus.png' height={15} width={15}/></span>
        <span className="text-md ml-1  ">Create New</span>
    </button>
</div>
